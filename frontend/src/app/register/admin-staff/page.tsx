...
import AdminStaffForm from "@/app/dashboard/admin-staff/AdminStaffForm";
import { getRegistrationSettings } from "@/lib/registrationSettings";

export default function PublicAdminStaffRegisterPage() {
  const [status, setStatus] = useState<"loading" | "open" | "closed" | "error">("loading");

  useEffect(() => {
    getRegistrationSettings()
      .then((s) => setStatus(s.admin_staff_registration_open ? "open" : "closed"))
    ...
    <div className="aasr-page">
      <div className="aasr-page-header">
        <div>
          ...
          <h1 className="aasr-page-title">Admin Staff Registration Form</h1>
      ...
      {status === "closed" && (
        ...
        >
          ...
          <p style={{ margin: 0, color: "var(--aasr-muted)" }}>
            Admin staff registration is not accepting new submissions right now. Please check back later.
          </p>
        </div>
      )}

      {status === "open" && <AdminStaffForm publicMode />}
    </div>
  );
}
