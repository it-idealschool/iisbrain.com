...
import TeacherForm from "@/app/dashboard/teachers/TeacherForm";
import { getRegistrationSettings } from "@/lib/registrationSettings";

export default function PublicTeacherRegisterPage() {
  const [status, setStatus] = useState<"loading" | "open" | "closed" | "error">("loading");

  useEffect(() => {
    getRegistrationSettings()
      .then((s) => setStatus(s.teacher_registration_open ? "open" : "closed"))
    ...
    <div className="aasr-page">
      <div className="aasr-page-header">
        <div>
          ...
          <h1 className="aasr-page-title">Teacher Registration Form</h1>
      ...
      {status === "closed" && (
        ...
        >
          ...
          <p style={{ margin: 0, color: "var(--aasr-muted)" }}>
            Teacher registration is not accepting new submissions right now. Please check back later.
          </p>
        </div>
      )}

      {status === "open" && <TeacherForm publicMode />}
    </div>
  );
}
