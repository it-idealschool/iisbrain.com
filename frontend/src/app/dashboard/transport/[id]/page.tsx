...
export default function TransportStaffProfilePage() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const data = await getTransportStaffMember(params.id);
      setStaff(data);
    } catch {
  ...
  return (
    <div className="aasr-page aasr-page--narrow print:mt-0">
      ...
      <div className="no-print aasr-page-header">
        <Link href="/dashboard/transport" className="aasr-btn aasr-btn-ghost aasr-btn-sm" style={{ paddingLeft: 0 }}>
          &larr; Back to Transport Staff
        </Link>
        <div className="aasr-actions">
          <button onClick={() => window.print()} className="aasr-btn aasr-btn-secondary">
            Print / Save as PDF
          </button>
          <Link href="/dashboard/transport" className="aasr-btn aasr-btn-primary">
            Edit
          </Link>
        </div>
      </div>
      ...
      <div className="print-card aasr-card" style={{ padding: "2rem" }}>
        <div style={{ display: "flex", gap: "1.5rem", alignItems: "center", borderBottom: "1px solid var(--aasr-border)", paddingBottom: "1.5rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
          ...
          <div>
            ...
            <p style={{ color: "var(--aasr-muted)", margin: "0.2rem 0 0" }}>
              {TRANSPORT_ROLE_LABELS[staff.role] || staff.role}
              {staff.is_substitute ? " · Substitute" : ""}
            </p>
          </div>
        </div>
        ...
        <section className="aasr-section">
          <h2 className="aasr-section-title">Transport Details</h2>
          <div className="aasr-field-grid">
            <div>
              <DetailRow label="License Number" value={staff.license_number} />
              <DetailRow label="Bus Number" value={staff.bus_number} />
              <DetailRow label="Route" value={staff.route} />
            </div>
            <div>
              <DetailRow
                label="Qualification"
        ...
        <section>
          <h2 className="aasr-section-title">Certificate</h2>
          {staff.certificate_url ? (
            ...
            <p style={{ fontSize: "0.86rem", color: "var(--aasr-muted)" }}>Not uploaded</p>
          )}
        </section>
      </div>
    </div>
